...
def init_odoo_agent():
    INIT_SCRIPT = """
    store <rtools> [{"type":"function","function":{"name":"message_notify_user","description":"The Communication Philosophy: Be a Autonomous Collaborator, Not a Silent Tool - You operate autonomously, but the user must always be aware of every step you take. Your core directive is partnership: you must communicate proactively, transparently, and continuously. The user should always know what you are reasoning, what you are doing, and any issues you encounter. The message tool is your only channel for notifications.  **Message types:**  1. **`info`**  - Progress and updates - Acknowledge user requests - Announce actions before executing tools - Report intermediate results or next steps - Explain errors and what you'll do next","parameters":{"properties":{"title":{"type":"string"},"message":{"type":"string"},"message_type":{"default":"info","type":"string"}},"required":["title","message"],"type":"object"}}},{"type":"function","function":{"name":"list_available_models","description":"Lists installed technical models (e.g., 'crm.lead', 'res.partner').  Args:     app_filter: (Optional) Filter by module name (e.g., 'crm', 'sale', 'account').","parameters":{"properties":{"app_filter":{"default":null,"type":"string"}},"type":"object"}}},{"type":"function","function":{"name":"inspect_model_fields","description":"Returns the table structure (fields, types, relationships, and help text).  Use this to: 1. Know the correct field names (e.g., is it 'partner_id' or 'client_id'?). 2. See which fields are required (required: true). 3. See accepted values in 'selection' fields.  Args:     model_name: The technical name of the model (e.g., 'sale.order').","parameters":{"properties":{"model_name":{"type":"string"}},"required":["model_name"],"type":"object"}}},{"type":"function","function":{"name":"execute_odoo_command","description":"Supported Commands: - 'search_read': To read data. Args: [[domain]], Kwargs: {'fields': [...], 'limit': 10} - 'create': To create records. Args: [{'field': 'value'}] - 'write': To edit. Args: [[ids], {'field': 'value'}] - 'call_method': To trigger workflow buttons (e.g., action_confirm). Method name goes in kwargs['method_name'].  SECURITY:  - Delete actions (unlink) are STRICTLY PROHIBITED. - If you need to remove something, try using 'action_archive' via call_method, if available.  Args:     model: Technical model (e.g., 'product.template').     command: 'search_read', 'create', 'write', or 'call_method'.     args: List of positional arguments.     kwargs: Named arguments. For call_method, must include 'method_name'.","parameters":{"properties":{"model":{"type":"string"},"command":{"type":"string"},"args":{"default":null,"items":{},"type":"array"},"kwargs":{"additionalProperties":true,"default":null,"type":"object"}},"required":["model","command"],"type":"object"}}},{"type":"function","function":{"name":"refresh_available_apps","description":"Use this if the user mentions a module that is not in your initial list or if they say 'I just installed module X'.","parameters":{"properties":{},"type":"object"}}}]
...
async def stream_chat_tests(req: ChatRequest):
    ...
    llm_instance <resp> instname n_predict=24 temperature=0.5 force=true stream=true tools=<rtools>
